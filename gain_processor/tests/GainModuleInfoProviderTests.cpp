...
class GainModuleInfoProviderTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(GainModuleInfoProviderTest, GetModuleInfo) {
    const GPUA::processor::v2::ModuleInfo* info;
    ASSERT_EQ(m_provider->GetModuleInfo(info), GPUA::processor::v2::ErrorCode::eSuccess);

    ASSERT_EQ(std::wstring(info->effect_name), L"GAIN");
    ASSERT_EQ(std::wstring(info->id), L"gain");

    // version is now always valid?
    // ASSERT_TRUE(info.version);
}

TEST_F(GainModuleInfoProviderTest, GetCodeSpecs) {
